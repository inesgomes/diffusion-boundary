...
class DatasetFactory:
    ...
    @staticmethod
    def dataset_from_lib(lib_name: str, model_name: str, dataset_name: str, n_classes: int, images: list):
        """Given the library and model name, return the corresponding dataset."""
        if lib_name == "transformers":
            return TransfomerDataset(dataset_name, n_classes, model_name, images)
        if lib_name in ("timm", "local"):
            return OtherDataset(dataset_name, n_classes, images)
